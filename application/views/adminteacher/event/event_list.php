  ...
  <div class="card">
    <?php $sat=$result['status'];  if($sat=="success"){ foreach($result['eventview'] as $rows1) {} ?>
                <div class="typo-line1">
                      ...
                      <blockquote>
                        <h5><?php echo $rows1->event_name; ?><span> <button onclick="history.go(-1);" class="btn btn-wd btn-default pull-right" style="margin-top:-10px;">BACK</button> </span> </h5> 
                       <p><?php echo $rows1->event_details; ?></p>
                       <p>Date: <?php echo $new_date = date('d-m-Y', strtotime($rows1->event_date)); ?></p>
                      </blockquote>
                  </div>
                  <?php }else{
